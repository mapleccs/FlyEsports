...
class TransferWindow(ValueObject):
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.window_name.strip()) > 100:
            raise BusinessRuleViolationError("Window name cannot exceed 100 characters")
        ...
        if duration.days > 90:
            raise BusinessRuleViolationError("Transfer window cannot exceed 90 days")
    ...
    def create_seasonal(
        ...
        if duration_days < 1 or duration_days > 90:
            raise BusinessRuleViolationError("Duration must be between 1 and 90 days")
