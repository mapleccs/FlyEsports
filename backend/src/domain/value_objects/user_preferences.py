...
class UserPreferences(ValueObject):
    ...
    def with_notifications(
        ...
        return UserPreferences(
            ...
            else self.email_notifications,
            push_notifications=push if push is not None else self.push_notifications,
            match_notifications=match
            if match is not None
            else self.match_notifications,
            rating_change_notifications=rating_change
            if rating_change is not None
            else self.rating_change_notifications,
            team_activity_notifications=team_activity
            if team_activity is not None
            else self.team_activity_notifications,
            profile_visibility=self.profile_visibility,
            show_real_name=self.show_real_name,
            show_email=self.show_email,
            items_per_page=self.items_per_page,
