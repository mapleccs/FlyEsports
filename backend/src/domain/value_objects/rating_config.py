...
class RatingConfig(ValueObject):
    ...
    Manages ELO calculation settings, confidence parameters, and rating constraints
    for a specific region.
    """
    
    # Base rating settings
    initial_rating: float = 50.0
    ...
    max_rating_difference: float = 15.0
    ...
    def __post_init__(self):
        ...
        if self.max_rating <= self.min_rating:
            raise BusinessRuleViolationError(
                f"Maximum rating ({self.max_rating}) must be greater than minimum rating ({self.min_rating})"
            )
        
        if not (self.min_rating <= self.initial_rating <= self.max_rating):
            raise BusinessRuleViolationError(
                f"Initial rating ({self.initial_rating}) must be between min ({self.min_rating}) and max ({self.max_rating})"
            )
        ...
        if self.min_confidence < 0 or self.min_confidence > 1:
            raise BusinessRuleViolationError(
                f"Minimum confidence must be between 0 and 1, got {self.min_confidence}"
            )
        
        if self.max_confidence < 0 or self.max_confidence > 1:
            raise BusinessRuleViolationError(
                f"Maximum confidence must be between 0 and 1, got {self.max_confidence}"
            )
        
        if self.max_confidence <= self.min_confidence:
            raise BusinessRuleViolationError(
                f"Maximum confidence ({self.max_confidence}) must be greater than minimum confidence ({self.min_confidence})"
            )
        
        if self.confidence_gain_per_match <= 0 or self.confidence_gain_per_match > 0.1:
            raise BusinessRuleViolationError(
                f"Confidence gain per match must be between 0 and 0.1, got {self.confidence_gain_per_match}"
            )
        
        if self.confidence_multiplier <= 0 or self.confidence_multiplier > 5:
            raise BusinessRuleViolationError(
                f"Confidence multiplier must be between 0 and 5, got {self.confidence_multiplier}"
            )
        
        # Validate placement matches
        if self.placement_matches < 0 or self.placement_matches > 50:
            raise BusinessRuleViolationError(
                f"Placement matches must be between 0 and 50, got {self.placement_matches}"
            )
        
        # Validate decay settings
        if self.decay_threshold_days <= 0:
            raise BusinessRuleViolationError(
                f"Decay threshold days must be positive, got {self.decay_threshold_days}"
            )
        
        if self.decay_rate_per_day < 0 or self.decay_rate_per_day > 5:
            raise BusinessRuleViolationError(
                f"Decay rate per day must be between 0 and 5, got {self.decay_rate_per_day}"
            )
        ...
        required_dimensions = {'kda', 'damage', 'economy', 'vision', 'objective', 'teamfight'}
        if set(self.dimension_weights.keys()) != required_dimensions:
            raise BusinessRuleViolationError(
                f"Dimension weights must contain exactly these keys: {required_dimensions}"
            )
        ...
        for dim, weight in self.dimension_weights.items():
            if not isinstance(weight, (int, float)):
                raise BusinessRuleViolationError(
                    f"Weight for dimension {dim} must be numeric, got {type(weight)}"
                )
            if weight < 0 or weight > 1:
                raise BusinessRuleViolationError(
                    f"Weight for dimension {dim} must be between 0 and 1, got {weight}"
                )
        ...
        if self.rating_range_multiplier <= 1.0 or self.rating_range_multiplier > 5.0:
            raise BusinessRuleViolationError(
                f"Rating range multiplier must be between 1.0 and 5.0, got {self.rating_range_multiplier}"
            )
        
        if self.max_rating_difference <= 0 or self.max_rating_difference > 50:
            raise BusinessRuleViolationError(
                f"Max rating difference must be between 0 and 50, got {self.max_rating_difference}"
            )
    ...
    def with_updated_dimension_weights(self, dimension_weights: Dict[str, float]) -> 'RatingConfig':
    ...
    def calculate_effective_k_factor(self, confidence: float, matches_played: int) -> float:
        """
    ...
    def should_apply_decay(self, days_inactive: int) -> bool:
        ...
        return (self.rating_decay_enabled and 
                days_inactive > self.decay_threshold_days)
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'RatingConfig':
        ...
        return cls(
            initial_rating=data.get("initial_rating", 50.0),
            min_rating=data.get("min_rating", 0.0),
            max_rating=data.get("max_rating", 100.0),
            ...
            max_rating_difference=data.get("max_rating_difference", 15.0)
        )
    
    def __str__(self) -> str:
        """String representation of rating configuration."""
        return (f"RatingConfig(initial={self.initial_rating}, "
                f"k={self.k_factor}, "
