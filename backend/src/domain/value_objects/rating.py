...
class Rating(ValueObject):
    ...
    # Rating constraints - using ELO rating system
    MIN_RATING = 0.0
    MAX_RATING = 5000.0
    MIN_CONFIDENCE = 0.5
    MAX_CONFIDENCE = 0.95

    # Six dimensions constraints (0-100 percentage scale)
    MIN_DIMENSION = 0.0
    MAX_DIMENSION = 100.0
    DEFAULT_DIMENSION = 50.0

    # Six dimensions keys
    DIMENSION_KEYS = {"kda", "damage", "economy", "vision", "objective", "teamfight"}
    ...
    def __post_init__(self) -> None:
        ...
        # Validate dimension values (0-100 percentage scale)
        for key, value in self.six_dimensions.items():
            ...
            if not (self.MIN_DIMENSION <= value <= self.MAX_DIMENSION):
                raise BusinessRuleViolationError(
                    f"Dimension {key} must be between {self.MIN_DIMENSION} and {self.MAX_DIMENSION}, "
                    f"got {value}"
                )

    @classmethod
    def create_initial(
        cls, initial_score: float, initial_dimensions: Optional[Dict[str, float]] = None
    ) -> "Rating":
        """
        Create initial rating for a new player.

        Args:
            initial_score: Initial rating score (ELO system, e.g., 1200)
            initial_dimensions: Initial six dimensions (optional, 0-100 scale)

        Returns:
            Rating value object
        """
        if initial_dimensions is None:
            # Default all dimensions to 50.0 (middle of 0-100 range)
    ...
    def with_new_score(self, new_score: float) -> "Rating":
        ...
        new_confidence = min(self.MAX_CONFIDENCE, self.confidence_level + 0.01)
    ...
    def effective_score(self) -> float:
        ...
        """
        return (
            self.locked_score if self.locked_score is not None else self.current_score
        )
    ...
    def __str__(self) -> str:
        ...
        return (
            f"Rating({self.effective_score:.1f}, "
            f"conf={self.confidence_level:.2f}, "
            f"matches={self.total_matches}){lock_status}"
        )
