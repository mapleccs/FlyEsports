...
class Team(AggregateRoot):
    ...
    def add_player(
        ...
        if self.roster[position] is not None:
            raise BusinessRuleViolationError(
                f"Position {position.value} is already occupied by {self.roster[position].player_name}"
            )
    ...
    def remove_player(
        ...
        if self.roster[position] is None:
            raise BusinessRuleViolationError(
                f"No player in position {position.value}"
            )
        ...
        object.__setattr__(self, 'total_cost', self.total_cost - removed_slot.signing_cost)
        object.__setattr__(self, 'updated_at', datetime.utcnow())
    ...
    def player_count(self) -> int:
        ...
        return sum(
            1 for slot in self.roster.values() 
            if slot is not None and slot.is_active
        )
    ...
    def active_players(self) -> List[RosterSlot]:
        ...
        return [
            slot for slot in self.roster.values() 
            if slot is not None and slot.is_active
        ]
    ...
    def __str__(self) -> str:
        ...
        return (f"Team({self.team_name} [{self.team_tag}], "
