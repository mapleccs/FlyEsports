...
class Region(AggregateRoot):
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.region_name) > 100:
            raise BusinessRuleViolationError("Region name cannot exceed 100 characters")
        ...
        if self.active_teams > self.total_teams:
            raise BusinessRuleViolationError("Active teams cannot exceed total teams")
        ...
        if not self.admin_users:
            raise BusinessRuleViolationError(
                "Region must have at least one administrator"
            )
    ...
    def remove_admin(self, user_id: str, removed_by_user_id: str) -> None:
        ...
        if len(self.admin_users) <= 1:
            raise BusinessRuleViolationError("Cannot remove the last administrator")
    ...
    def start_season(
        self, season_name: str, start_time: datetime, end_time: datetime
    ) -> None:
        """
        Start a new season.
        ...
        if len(season_name.strip()) > 50:
            raise BusinessRuleViolationError("Season name cannot exceed 50 characters")
    ...
    def __str__(self) -> str:
        ...
        return (
            f"Region({self.region_name} [{self.region_code}], "
