...
class Settings(BaseSettings):
    ...
    RIOT_API_BASE_URL: str = Field(
        default="https://api.riotgames.com",
        description="Riot API base URL"
    )
    ...
    MIN_DB_CONNECTIONS: int = Field(default=5, description="Minimum database connections")
    ...
    RIOT_API_RATE_LIMIT: int = Field(default=100, description="Riot API rate limit per minute")
    ...
    ENABLE_RATING_UPDATES: bool = Field(default=True, description="Enable rating updates")
    ...
    @field_validator("MAX_DB_CONNECTIONS", "MIN_DB_CONNECTIONS", "REDIS_MAX_CONNECTIONS")
    @classmethod
    ...
    def validate_production_settings(cls, values: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if env == "production":
            ...
            if jwt_secret in ["your-secret-key-change-in-production", "dev-jwt-secret-key"]:
...
def validate_settings(settings_instance: Settings) -> bool:
    ...
    try:
        ...
        if settings_instance.ENVIRONMENT != "test":
            if "localhost" in settings_instance.DATABASE_URL and settings_instance.ENVIRONMENT == "production":
                logger.warning("Using localhost database in production environment")
        
        # Test Redis connection format
        if "localhost" in settings_instance.REDIS_URL and settings_instance.ENVIRONMENT == "production":
            logger.warning("Using localhost Redis in production environment")
        
        # Validate Celery configuration
        if settings_instance.CELERY_BROKER_URL == settings_instance.CELERY_RESULT_BACKEND:
            logger.info("Using same Redis instance for Celery broker and result backend")
        ...
        logger.info(
            ...
            database_pool_size=f"{settings_instance.MIN_DB_CONNECTIONS}-{settings_instance.MAX_DB_CONNECTIONS}",
...
def get_environment_info() -> Dict[str, Any]:
    ...
    return {
        ...
        "environment_variables": {
            key: "***" if any(secret in key.lower() for secret in ["password", "secret", "key", "token"]) else value
            for key, value in os.environ.items()
            if key.startswith(("FLYESPORTS_", "DATABASE_", "REDIS_", "JWT_", "CELERY_", "RIOT_"))
        },
