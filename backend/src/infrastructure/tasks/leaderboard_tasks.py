...
from src.domain.services.leaderboard_service import LeaderboardService
    ...
    try:
        async def _update_leaderboard() -> Dict[str, Any]:
            async with database_manager.get_session() as session:
                ...
                positions_to_update = positions or ["TOP", "JUNGLE", "MIDDLE", "BOTTOM", "UTILITY", None]
                updated_positions = []
        ...
        logger.error(
            "Region leaderboard update failed",
            region_id=region_id,
            exception=str(exc)
        )
        ...
        async def _create_snapshots() -> Dict[str, Any]:
            async with database_manager.get_session() as session:
                ...
                for region in regions:
                    ...
                    try:
                        snapshot = await leaderboard_service.create_leaderboard_snapshot(
                            region.id, "daily"
                        )
    ...
    try:
        async def _warm_cache() -> Dict[str, Any]:
            async with database_manager.get_session() as session:
                ...
                positions_to_warm = positions or ["TOP", "JUNGLE", "MIDDLE", "BOTTOM", "UTILITY", None]
                warmed_entries = []
        ...
        logger.error(
            "Leaderboard cache warm-up failed",
            region_id=region_id,
            exception=str(exc)
        )
        raise self.retry(exc=exc)