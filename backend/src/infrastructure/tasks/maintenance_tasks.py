        ...
        async def _cleanup() -> Dict[str, Any]:
            async with database_manager.get_session() as session:
                ...
                cleanup_summary["operations"].append({
                    ...
                    "cleaned_count": expired_sessions
                })
                
                # Clean up old temporary data
                old_temp_data = await _cleanup_old_temp_data(session)
                cleanup_summary["operations"].append({
                    "operation": "old_temp_data", 
                    "cleaned_count": old_temp_data
                })
                
                # Clean up old event logs
                old_events = await _cleanup_old_events(session)
                cleanup_summary["operations"].append({
                    "operation": "old_event_logs",
                    "cleaned_count": old_events
                })
                
                # Clean up Redis cache
                cache_cleanup = await _cleanup_redis_cache()
                cleanup_summary["operations"].append({
                    "operation": "redis_cache_cleanup",
                    "cleaned_keys": cache_cleanup
                })
    ...
    try:
        async def _optimize() -> Dict[str, Any]:
            async with database_manager.get_session() as session:
                ...
                tables_to_analyze = [
                    "users", "player_profiles", "teams", "matches",
                    "ratings", "leaderboards", "transfers"
                ]
