...
class Team(BaseModel):
    ...
    # 队长信息
    captain_user_id: Mapped[int] = mapped_column(ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    
    # 队伍状态
    is_active: Mapped[bool] = mapped_column(Boolean, default=True, nullable=False)
    is_recruiting: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)  # 是否正在招募
    ...
    registrations: Mapped[List["SeasonRegistration"]] = relationship("SeasonRegistration", back_populates="team", cascade="all, delete-orphan")
    team_a_matches: Mapped[List["Match"]] = relationship("Match", foreign_keys="Match.team_a_id", back_populates="team_a")
    team_b_matches: Mapped[List["Match"]] = relationship("Match", foreign_keys="Match.team_b_id", back_populates="team_b")
...
class TeamMember(BaseModel):
    ...
    # 成员信息
    position: Mapped[Optional[str]] = mapped_column(String(20), nullable=True)  # 位置：top, jungle, mid, adc, support
    is_substitute: Mapped[bool] = mapped_column(Boolean, default=False, nullable=False)  # 是否为替补
