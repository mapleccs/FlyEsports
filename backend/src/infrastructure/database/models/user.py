...
class User(BaseModel):
    ...
    riot_summoner_name: Mapped[Optional[str]] = mapped_column(String(100), nullable=True, index=True)
    ...
    managed_regions: Mapped[List["Region"]] = relationship("Region", back_populates="admin_user")
    captained_teams: Mapped[List["Team"]] = relationship("Team", back_populates="captain")
...
class UserRole(BaseModel):
    ...
    region_id: Mapped[Optional[int]] = mapped_column(ForeignKey("regions.id", ondelete="CASCADE"), nullable=True)  # NULL表示全局角色
    
    # 角色生效时间
    granted_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    expires_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)  # NULL表示永不过期
    ...
    region: Mapped[Optional["Region"]] = relationship("Region", back_populates="user_roles")
