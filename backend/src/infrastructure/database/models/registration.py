...
class SeasonRegistration(BaseModel):
    ...
    # 报名状态
    status: Mapped[RegistrationStatus] = mapped_column(Enum(RegistrationStatus), default=RegistrationStatus.PENDING, nullable=False, index=True)
    ...
    def __repr__(self) -> str:
        return f"<SeasonRegistration(id={self.id}, season_id={self.season_id}, team_id={self.team_id}, status='{self.status.value}')>"
